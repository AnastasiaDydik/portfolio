...
function extractNameFromTemplate(value) {
    var result = value.slice(value.indexOf(" ") + 1, value.indexOf("!"));
    return result;
}
...
function getCardId(value) {
       var firstSymbol = value.slice(0, value.length-1),
        lastSymbol = value.charAt(value.length-1),
        result = 0;

    if (lastSymbol === "♣") { result += 0; }
        else if (lastSymbol === "♦") { result += 13; }
        else if (lastSymbol === "♥") { result += 26; }
        else if (lastSymbol === "♠") { result += 39; }
    if (firstSymbol === "A") { result += 0; }
        else if (firstSymbol === "J") { result += 10; }
        else if (firstSymbol === "Q") { result += 11; }
        else if (firstSymbol === "K") { result += 12; }
        else { result += parseInt(firstSymbol)-1; }
