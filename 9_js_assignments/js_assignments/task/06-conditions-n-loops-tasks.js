...
function isInsideCircle(circle, point) {
    var distance = Math.sqrt( Math.pow(Math.abs(circle.center.x - point.x), 2) + Math.pow(Math.abs(circle.center.y - point.y), 2) );
...
function isBracketsBalanced(str) {
    ...
    openBrackets = [],
    isClean = true;
    for(var i = 0; isClean && i<str.length; i++ ) {
        if( bracket[str[i]] ) {
...
function getCommonDirectoryPath(pathes) {
  ...
  for (var i = 0; i < pathes[0].length; i++) {
    symb = pathes[0].charAt(i);
    for (var j = 1; j < pathes.length; j++) {
      if (pathes[j].charAt(i) !== symb) {
        j = pathes.length + 1;
        flag = false;
      }
    }
